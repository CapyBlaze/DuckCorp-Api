...
export async function playersByProduction(page: number = 1, pageSize: number = 10) {
    ...
    const playersWithProduction = await Promise.all(
...
export async function playersByStorage(page: number = 1, pageSize: number = 10) {
    ...
    const playersWithStorage = await Promise.all(
        players.map(async (player) => {
...
export async function getPlayerRank(playerId: string) {
    ...
    const storageList = await Promise.all(
        allPlayers.map(async (p) => ({
            id: p.id,
    ...
    const buildingsList = buildingsPlayers
        .map((p) => ({
            id: p.id,
    ...
    const storagesList = storagesPlayers
        .map((p) => ({
            id: p.id,
